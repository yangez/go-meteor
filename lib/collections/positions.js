...
Positions.new = function(gameId, posAttributes) {
  ...
  if (posAttributes) insert = _.extend(insert, posAttributes);

  insert = _.extend(insert, positionMethods); // so we can use Position methods on this before we save it to Mongo
...
Positions.helpers( positionMethods );

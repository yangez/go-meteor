...
Games.deny({
  update: function(userId, game, fieldNames) {
    return (_.without(fieldNames, 'blackPlayerId', 'whitePlayerId', 'wgoGame', 'boardState', 'messages', 'archived', 'previousMarker', 'markedDead', 'markedSchema', 'originalBoardState', 'userAcceptedMD', 'lastActivityAt', 'endedAt').length > 0);
  }
})
...
Meteor.methods({
  ...
  joinGame: function(game, color, userId) {
    ...
    if (game.blackPlayerId && game.whitePlayerId) {
      pushMessage(game, "Game has started, "+game.playerToMove().username+" begins. Enjoy!", GAME_MESSAGE);
    }

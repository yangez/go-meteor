...
Games.deny({
  update: function(userId, game, fieldNames) {
    return (_.without(fieldNames, 'blackPlayerId', 'whitePlayerId', 'wgoGame', 'boardState', 'messages', 'archived', 'previousMarker').length > 0);
  }
})
...
Meteor.methods({
  ...
  joinGame: function(game, color, userId) {
    ...
    if (color === "black") {
      ...
      // Games.update({_id: game._id}, {$set: { whitePlayerId: userId} }, {$push: { players: userId } }) // if we need an extra variable
    }
    return { _id: game._id }
  },

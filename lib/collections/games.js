...
Games.helpers({
  ...
  togglePointAsDead: function(x, y) {
    ...
    // if point is the same as the original (and stone exists): set point to neutral in marked
    if (
      marked[clickedIndex] === original[clickedIndex] &&
      [WGo.B, WGo.W].indexOf(marked[clickedIndex]) > -1
    ) {
      marked[clickedIndex] = 0; action = "push";
    }
      ...
      set point to the original in marked */
        ...
        marked[clickedIndex] = original[clickedIndex]; action = "pull";
      }

      // write to DB if something changed
      // action can either be "push" or "pull" depending on above
      if (action) {
        var query = {
        ...
        query["$"+action] = {deadMarkers: marker};
        Games.update({_id: game._id}, query);

        // example of pushing multiple:

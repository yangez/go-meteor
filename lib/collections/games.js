...
Games.helpers({
  ...
  endGame: function(method, attr) {
    ...
    if (Meteor.isServer) {
      ...
      // winner
      var eloApi = Meteor.npmRequire('elo-rank');
      console.log("hi world!!")
      // var eloRankA = Meteor.npmRequire('elo-rank')(winner.getEloK());


      var expectedScoreA = eloRankA.getExpected(winnerRating, loserRating);
      var winnerNewRating = eloRankA.updateRating(expectedScoreA, 1, winnerElo);

      winner.updateRating(winnerNewRating);

      // loser
      var eloRankB = Meteor.npmRequire('elo-rank')(loser.getEloK());

      var expectedScoreB = eloRankB.getExpected(loserRating, winnerRating);
      var loserNewRating = eloRankB.updateRating(expectedScoreB, 0, loserElo);

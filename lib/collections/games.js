...
Games.helpers({
  ...
  playMove: function(x, y) {
    ...
    var set = {
      ...
      lastActivityAt: new Date(),
      undoRequested: false,
      previousPositionId: formerPosition._id,
      turn: this.turn
    };
  ...
  acceptUndo: function() {
    ...
    var set = {
      turn: setColor,
      previousPositionId: this.position()._id,
    }
    var unset = {
      undoRequested: "",
      turnMarker: "" ,
    ...
    // remove the appropriate position
    Positions.remove({gameId: this._id}, {
      sort: {createdAt: -1},
      limit: 1
    })
 ...
 */
  ...
  pushMessage: function(content, type) {
    ...
    if (this.currentMove()) currentMove = this.currentMove()-1;
 ...
 */
  ...
  currentMove: function() {
    if (this.isReady() || this.markingDead()) return this.positionCount();
  },
  ...
  checkHistory: function(position, x, y) {
    ...
    var positionCount = this.positionCount();
  	...
  	for(var i = positionCount-1; i >= stop; i--) {
  		if(this.positionAt(i-1) && this.positionAt(i-1).get(x,y) == position.get(x,y)) {
  			flag = true;
  			for(var j = 0; j < this.size*this.size; j++) {
  				if(this.positionAt(i-1) && this.positionAt(i-1).schema[j] != position.schema[j]) {
  					flag = false;
  					break;
  				}
  			}

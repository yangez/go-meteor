...
Games.helpers({
  ...
  // normal, time, resignation
  endGame: function(method, attr) {
    if (["normal", "time", "resignation"].indexOf(method) === -1) throw new Meteor.Error("endGame method '"+method+"' not found.");

    var archiveMethod, score, winner, loser, message;

    if (method === "normal") {
      score = this.getFinalScore();
      winner = this.getWinner();
      loser = this.getLoser();
      message = "Game ended. Final score: "+score+". Congratulations "+winner.username+"!";
      archiveMethod = "finished";
    }
    else if (method === "time") {
      var color = attr.color;
      loser = this.getPlayerAtColor(color);

      if (!loser || !this.timerIsRunning()) return false;
      if (this.absTimeLeft(color) > 0 ) return false;
      var winnerId = this.getOtherPlayerId(loser._id);

      winner = Meteor.users.findOne(winnerId);
      score = (color === "white") ? "B+" : "W+"
      message = "Game ended, "+loser.username+" ran out of time. Congratulations "+winner.username+"!";
      archiveMethod = "timedout";
    }
      ...
      score = (loserColor === "white") ? "B+" : "W+"

      var winnerColor = getOppositeColor(loserColor);
      winner = this.getPlayerAtColor(winnerColor);
      message = loser.username + " resigned. Congratulations "+winner.username+"!";
      archiveMethod = "resigned";
    }


    var set = {
      archived: archiveMethod,
      endedAt: new Date(),
      score: score,
      winnerId: winner._id,
      loserId: loser._id
    ...
    if (Games.update({_id: this._id}, {$set: set})) {

      this.setPlayerElo(winner, loser);
  ...
  // set player elo after game ended
  setPlayerElo: function(winner, loser) {
    ...
    // winner
    var eloRankA = new Elo(winner.getEloK());
    var expectedScoreA = eloRankA.getExpected(winnerRating, loserRating);
    var winnerNewRating = eloRankA.updateRating(expectedScoreA, 1, winnerRating);

    winner.updateRating(winnerNewRating);

    // loser
    var eloRankB = new Elo(loser.getEloK());

    var expectedScoreB = eloRankB.getExpected(loserRating, winnerRating);
    var loserNewRating = eloRankB.updateRating(expectedScoreB, 0, loserRating);

    loser.updateRating(loserNewRating);
  ...
  getFinalScore: function() {
    ...
    if (this.markedSchema) {
      var markedPosition = Positions.new(this._id);
  ...
  playMove: function(x, y) {
    ...
    if (x==="pass") { // if we're playing a pass
      ...
      // var turn = (this.turn === WGo.B) ? WGo.W : WGo.B;
  ...
  togglePointAsDead: function(x, y) {
    ...
    var clickedIndex = convertCoordinatesToSchemaIndex(original, x, y);

    // return if stone doesn't exist on point
    if (!clickedIndex || [WGo.B, WGo.W].indexOf(original[clickedIndex]) === -1) return false;

    // stone exists and it's the same as original: we need to mark dead
    if ( marked[clickedIndex] === original[clickedIndex] && [WGo.B, WGo.W].indexOf(marked[clickedIndex]) > -1 )
      action = "push";

    // stone exists and it's different than original: we need to remove mark
    else if (marked[clickedIndex] != original[clickedIndex])
      action = "pull";

    if (action) {

      // find all adjacent stones
      var adjacentIndices = game.position().traverseStones(clickedIndex);
      var markers = [];

      adjacentIndices.forEach(function(stoneIndex){
        var coord = convertSchemaIndexToCoordinates(original, stoneIndex);

        var marker = { x: coord.x, y: coord.y, type: "DEAD" }
        markers.push(marker);

        if (action === "push") marked[stoneIndex] = 0;
        else if (action === "pull") marked[stoneIndex] = original[stoneIndex];
      });

      var query = {
        $set: { markedSchema: marked },
        $unset: { userAcceptedMD: "" },
      }

      if (action === "push") query["$push"] = { deadMarkers: {$each: markers} }
      else if (action === "pull") query["$pullAll"] = { deadMarkers: markers }

      Games.update({_id: game._id}, query);
  ...
  // someone resigns
  resign: function() {
    return this.endGame("resignation");
  },
  ...
  acceptUndo: function() {
    ...
    Positions.remove(this.position()._id);
 ...
 */
  ...
  pushMessage: function(content, type) {
    ...
    if (this.currentMove() >= 0) currentMove = this.currentMove();
 ...
 */
  ...
  getColorOfCurrentMove: function() {
    ...
    if (this.turn === 1) return "black";
    else if (this.turn === -1) return "white";
  },
...
*/
  ...
  /**
  *
  * Get cursor of all positions
  *
  */
  positions: function() {
    return Positions.find({gameId: this._id}, {sort: {createdAt: -1}});
  },
  ...
  */
    ...
    if (turn < 1 || turn > this.positionCount()) return false;
	...
	wgoPlay: function(x,y,c,noplay) {
		...
		// check suicide
		if(!captured.length) {
			var testing = Positions.new(this._id);
	...
	isValid: function(x,y,c) {
		return typeof this.wgoPlay(x,y,c,true) != "number";
	},
  ...
  checkHistory: function(position, x, y) {
    ...
    var positionCount = this.positionCount();
      ...
      var previousPosition = this.positionAt(i);
  		if(previousPosition && previousPosition.get(x,y) == position.get(x,y)) {
  			flag = true;
  			for(var j = 0; j < this.size*this.size; j++) {
  				if(previousPosition && previousPosition.schema[j] != position.schema[j]) {
  					flag = false;
  					break;
  				}
  			}

  ...
  pushMessage: function(message, user) {
    ...
    if (user) {
      ...
      if (user.gameMessage) {
        ...
        // if it's a pass, also assign a move number
        if (message.indexOf("has passed") > -1) currentMoveNumber = this.getCurrentMoveNumber();
      }
      // if it's a user message
      else if (user.username) {
        username = user.username;
        currentMoveNumber = this.getCurrentMoveNumber();
      }

 ...
 */
  ...
  checkTimerFlag: function() {
    ...
    var whiteTimeRemaining = game.absTimeRemaining("white");
    var blackTimeRemaining = game.absTimeRemaining("black");

    if (game.absTimeRemaining("white") === 0)
      Meteor.call("game/endOnTime", game._id, "white");
    else if (game.absTimeRemaining("black") === 0)
  ...
  byoTimeRemaining: function(color) {
    ...
    // if it's the first move in the game
      ...
      return game.byoTotalTime();

    // if it's this person's turn and timer is running
    else {
      var timeBank = this.byoLeft[color] * this.byoyomi.time;
      var remaining = timeBank - this.timeSinceLastMove();
      return (remaining < 0) ? 0 : remaining;
    }
  },

  byoFormatTimeRemaining: function(color) {
    if (this.isTimed() !== "byoyomi") return false;

    var user = this.getPlayerAtColor(color);
    if (!this.hasPlayer(user)) return false;

    var timeRemaining = this.byoTimeRemaining(color);

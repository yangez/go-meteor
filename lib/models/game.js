 ...
 */
  ...
  totalTimeLeft: function(color) {
    ...
    if (this.isTimed() === "byoyomi") debugger;

    var absTimeBank = this.gameLength - this.timeUsed[color];
    ...
    else if (this.getColorOfCurrentMove() !== color || this.markingDead()) {
  ...
  byoFormatTimeLeft: function(color) {
    ...
    var periods = this.byoLeft[color] - parseInt( (this.byoTotalTime() - timeLeft) / this.byoyomi.time );
  ...
  byoIsRunning: function(color) {
    if (this.isTimed() !== "byoyomi") return false;
    return this.absTimeLeft(color) === 0;
  },

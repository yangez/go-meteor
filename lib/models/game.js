...
_.extend(Game.prototype, {
  ...
  createGame: function(size, repeat) {
    // use 9 if size is invalid
    if (["9", "13", "19"].indexOf(size) === -1) size = 9;
    if (!this.wgoGame) {
      var wgoGame = new WGo.Game(size, repeat);
      Games.update({_id: this._id }, { $set: { wgoGame: wgoGame.exportPositions(), messages:[], lastActivityAt: new Date() } });
    }
    return this;
  },
  ...
  getFinalScore: function() {
    ...
    if (this.markedSchema) {
      var markedPosition = _.clone(this.wgoGame.getPosition());
 ...
 */
  ...
  pushMessage: function(message, user) {
    ...
    else // push to local collection (non-persistent)
      return Games._collection.update({_id: this._id}, {$push: {messages: messageObj}})
  },

  getMessagesBeforeMove: function(moveNumber) {
    // get bulk of messages
    var messages = _.filter(this.messages, function(m) { return m.currentMove < moveNumber; });

    // add beginning game message
    /*
    var beginningMessageIndex = _.findIndex(this.messages, function(m) { return m.class === "game-message"; });
    var beginningMessageObject = this.messages[beginningMessageIndex];
    messages.unshift(beginningMessageObject);
    */

    // add ending message on
    /*
    if (moveNumber >= this.wgoGame.stack.length-1) {
      var endingMessageIndex = _.findLastIndex(this.messages, function(m) { return m.class === "game-message"; });
      var endingMessageObject = this.messages[endingMessageIndex];
      messages.push(endingMessageObject);
    }
    */

...
_.extend(Game.prototype, {
  ...
  playMove: function(x, y) {
    ...
    if (this.isTimed()) {
      ...
      set = _.extend(set, { lastMoveAt: new Date });
 ...
 */
  ...
  // returns whether this game is timed
  isTimed: function() {
    return this.timeUsed && this.gameLength ? true : false;
  },
  ...
  // returns the time remaining for a color in milliseconds
  timeRemaining: function(color) {
    if (!this.isTimed()) return false;

    var user = this.getPlayerAtColor(color);
    if (!this.hasPlayer(user)) return false;

    // if this color is going
    if (this.getColorOfCurrentMove() === color) {
      var remaining = this.gameLength - this.timeUsed[color] - this.timeSinceLastMove();
      return (remaining < 0) ? 0 : remaining;

    } else {
      // return game length - the time they've used
      return this.gameLength - this.timeUsed[color];
    }
  },

  // check if time expired for a color
  timeExpired: function(color) {
    if (!this.isTimed()) return false;
    var user = this.getPlayerAtColor(color);
    if (!this.hasPlayer(user)) return false;
  },
  ...
  timeSinceLastMove: function() {
    if (!this.isTimed()) return false;
    return moment() - moment(this.lastMoveAt);
  },
